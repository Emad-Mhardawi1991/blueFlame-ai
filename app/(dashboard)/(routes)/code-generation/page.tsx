    ...
    <div className="max-w-[1400px] m-auto">
      ...
      <div>
        ...
        {chats.map((chat, index) => (
          <div
            key={index}
            className="p-4 lg:px-8 bg-gray-900 m-6 rounded-lg "
          >
            <p className="text-xl font-bold pb-4 text-green-400">{chat.question}</p>
           <ReactMarkdown 
         
           components={{
            pre: ({node, ...props})=>(
              <div className="overflow-auto w-full my-2 bg-black/50 text-white p-2 rounded-lg" >
                <pre {...props}/>
              </div>
            ),
            code:({node, ...props})=>(
              <code {...props} className="bg-black/50 font-bold"/>
                )
           }}
